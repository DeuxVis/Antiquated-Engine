...
DWORD WINAPI LoopbackCaptureThreadFunction(LPVOID pContext) {
    ...
    if (FAILED(pArgs->hr)) {
        SysDebugPrint( "CoInitialize failed: hr = 0x%08x", pArgs->hr);
        return 0;
    }
...
HRESULT LoopbackCapture(
    ...
    if (FAILED(hr)) {
        SysDebugPrint( "IMMDevice::Activate(IAudioClient) failed: hr = 0x%08x", hr);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        SysDebugPrint( "IAudioClient::GetDevicePeriod failed: hr = 0x%08x", hr);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        SysDebugPrint( "IAudioClient::GetMixFormat failed: hr = 0x%08x", hr);
        return hr;
    }
    ...
    if (bInt16) {
        ...
        switch (pwfx->wFormatTag) {
            ...
            case WAVE_FORMAT_EXTENSIBLE:
                {
                    ...
                    if (IsEqualGUID(KSDATAFORMAT_SUBTYPE_IEEE_FLOAT, pEx->SubFormat)) {
                        ...
                        SysDebugPrint( "Don't know how to coerce mix format to int-16");
                        return E_UNEXPECTED;
                    }
                }
                break;
            ...
            default:
                SysDebugPrint( "Don't know how to coerce WAVEFORMATEX with wFormatTag = 0x%08x to int-16", pwfx->wFormatTag);
                return E_UNEXPECTED;
        }
    }
    ...
    *pnFrames = 0;
    ...
    if (FAILED(hr)) {
        SysDebugPrint( "IAudioClient::Initialize failed: hr = 0x%08x", hr);
        return hr;
    }
    ...
    if (FAILED(hr)) {
        SysDebugPrint("IAudioClient::GetService(IAudioCaptureClient) failed: hr = 0x%08x", hr);
        return hr;
    }
    ...
    */
	...
	{
        DWORD dwErr = GetLastError();
        SysDebugPrint( "SetWaitableTimer failed: last error = %u", dwErr);
        return HRESULT_FROM_WIN32(dwErr);
    }
    ...
    if (FAILED(hr)) 
	{
        SysDebugPrint( "IAudioClient::Start failed: hr = 0x%08x", hr);
        return hr;
    }
